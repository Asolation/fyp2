        ...
        <div class="card shadow">
            <div class="card-header">
                <div class="d-sm-flex align-items-center justify-content-between mb-4">
                    <h1 class="h3 mb-0 text-gray-800">{{ __('Create Quizzes') }}</h1>
                    <a href="{{ route('admin.quizzess.index') }}" class="btn btn-primary btn-sm shadow-sm">{{ __('Go Back') }}</a>
                </div>
            </div>
            <div class="card-body">
                <form action="{{ route('admin.quizzess.store') }}" method="POST">
                    @csrf
                    ...
                    <div class="form-group">
                        ...
                        <input type="number" class="form-control" id="time" placeholder="{{ __('Time') }}" name="time" value="{{ old('time') }}" />
                    </div>
                    <div class="form-group">
